...
- 整頁背景採分層式資產策略：少數具象場景使用圖片資產，中後段優先使用 CSS 漸層、色溫、手記/紙感材質、細線圖形、陰影與既有圖片形成章節差異。
- 不預設為每個 section 新增 full-bleed AI 大圖；新增大型背景圖前需確認敘事必要性、桌機/手機構圖、壓縮後大小、載入優先級與是否會增加視覺噪音。
- 若使用同一張 ambient 背景作為世界觀底色，仍需透過 section 背景層、分隔線、材質或局部圖形維持段落辨識，不讓整頁變成同一張布幕。
...
- [x] 提供繁體中文與英文內容，並以輕量 TypeScript 切換語系相關狀態與連結。
- [ ] 是否需要作品資料抽成 JSON 或 Markdown。
- [ ] 是否需要動態主題、canvas、WebGL 或其他高客製互動。
- [x] 第一版正式公開的聯絡方式與外部連結。
- [x] 正式作品由專業技能區中依目前語系前往對應版本的外部作品集。
