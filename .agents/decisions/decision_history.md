...
### D-2026-06-13-001 - 長頁背景採窗邊手記式分層策略

- **日期**：2026-06-13
- **狀態**：已確認
- **觸發來源**：使用者檢視整頁背景時指出首屏大視覺與第二段溫馨兔子背景品質良好，但後續區塊整體看起來較像填塞氛圍；同時擔心大量 AI 大圖造成畫面複雜與載入負擔，也擔心同一背景一路跟隨滾動會讓區塊分隔不明。
- **決策內容**：
  - 整頁背景採「窗邊手記式長頁」策略：前兩段保留清楚具象場景，中後段以章節色溫、手記材質、輕量 CSS 圖形與少量既有圖片作為視覺節奏。
  - 不為每個 section 新增 full-bleed AI 大圖；大型具象圖只用在真正需要建立場景或情緒高潮的位置。
  - 不讓同一張溫馨背景無差別固定覆蓋整頁；每個區塊需要有可辨識的分隔、色溫與語意材質。
  - 後續背景調整優先使用 CSS custom properties、漸層、細線、紙感、相簿/手記/路徑等低成本語彙；新增圖片資產前需確認它真的提升敘事，而不是補裝飾。
  - 首屏與第二段仍維持目前的高辨識度主視覺；後續區塊應像同一本手記裡不同頁面，而不是彼此無關的海報或模板卡片。
  - 關係／愛的道路段落是後半頁情緒高潮，可使用壓縮後、低對比、非人物、非直白符號的生成式氛圍背景，但必須以深色遮罩與既有 CSS 語彙融入頁面。
  - 關係段一旦使用大型背景圖，必須和首屏/第二段一樣維持桌機橫式與手機直式兩套構圖資產，不可用桌面圖在手機版放大裁切代替手機視覺設計。
- **理由**：本網站需要保持精緻、個人、非模板的視覺敘事，但同時維持輕量靜態站的速度與可維護性。大量大圖會增加載入、構圖與視覺噪音風險；單一背景一路到底又會削弱段落節奏。分層式背景系統能兼顧章節辨識、整體一致性與靜態效能。
- **影響範圍**：
  - `src/styles/main.css`
  - `README.md`
  ...
  - 後續所有 section 背景、視覺資產、CSS 材質與頁面節奏調整
- **後續 Agent 行動**：
  - 新增或修改 section 背景前，先判斷該區是具象場景、章節氛圍帶、相簿/手記頁，還是安靜收尾。
  - 優先沿用現有背景系統與輕量 CSS 語彙；不要為了讓單段更華麗而破壞整頁節奏。
  - 若要新增大型 AI 背景圖，需先確認它的敘事角色、桌機/手機構圖、壓縮策略與是否值得增加資產重量。
  - 關係段背景若未來替換，應同時替換桌機與手機版本，並維持月光、窗光、絲線、花瓣或路徑等抽象敘事，不加入人物、身體、愛心、標語或明顯情色圖像。

### D-2026-06-12-001 - 視覺檢查只由使用者進行

- **日期**：2026-06-12
- **狀態**：已確認
- **觸發來源**：圖片資產與載入策略檢查後，使用者明確要求「以後這個專案的視覺檢查由我來進行就好，紀錄進文件告訴後續agent」。
- **決策內容**：
  - 本專案的視覺檢查、審美判斷、畫面構圖確認與跨裝置實際觀感，皆由使用者本人進行。
  - Agent 不應主動啟動 in-app Browser、Edge/Chromium headless、自動截圖或其他瀏覽器視覺檢查流程來替代使用者。
  - Agent 仍應負責非視覺的機械驗證，例如 `npm run build`、typecheck、資產路徑、HTML/CSS/DOM 結構、圖片尺寸屬性、lazy/fetch priority 設定、可及性屬性與明顯 overflow/CLS 風險。
  - 若需要確認視覺結果，Agent 應把變更與可預覽狀態交給使用者，等待使用者的手動瀏覽器回饋後再修正。
  - 使用者提供截圖或文字描述時，以使用者看到的實際畫面為準；Agent 不用自動截圖反向驗證或推翻使用者回饋。
- **理由**：本網站是高度個人化的靜態自我介紹，視覺品質與氛圍判斷以專案擁有者的實際瀏覽器與審美為準。先前自動瀏覽器與 headless 截圖在本機 Windows/Codex 環境中已有可信度與啟動限制，繼續要求 Agent 做視覺檢查會增加噪音並拖慢迭代。
- **影響範圍**：
  - `AGENTS.md`
  - `.agents/skills/professional/development_standards.md`
  - `.agents/architecture/technical_architecture.md`
  - 所有後續 UI、responsive layout、圖片、動效、視覺資產與設計迭代任務
- **後續 Agent 行動**：
  - UI 或圖片相關變更後，只回報已完成的 build/DOM/CSS/資產/屬性檢查。
  - 不因沒有自動截圖或瀏覽器視覺檢查而阻塞任務。
  - 明確請使用者用自己的瀏覽器檢查視覺結果，並依使用者回饋調整。

### D-2026-06-11-007 - UI 視覺品質以使用者手動瀏覽器檢驗為準

- **日期**：2026-06-11
- **狀態**：已由 `D-2026-06-12-001` 修訂
- **觸發來源**：職業技能區塊實作後，使用者直接以自己的瀏覽器檢驗畫面，指出自動截圖無法有效承接實際視覺判斷，並明確要求「以後都由我來手動檢驗」。
- **決策內容**：
  - 後續 UI、版面、響應式與視覺品質檢驗，以使用者手動瀏覽器檢驗與截圖回饋為準。
  - Agent 應負責執行可機械驗證的工作，例如 `npm run build`、typecheck、資產是否能被 served、HTML/CSS 結構與明顯路徑錯誤檢查。
  - Agent 不應反覆嘗試 in-app Browser、Edge/Chromium headless 或其他自動截圖來替代使用者的視覺判斷；若需要確認畫面，應交付可預覽狀態並等待使用者回饋。
  - 若使用者提供截圖或文字回饋，應直接依該回饋修正，不再以 headless 截圖結果反向推翻使用者看到的畫面。
- **理由**：本專案重視個人化視覺與細膩氛圍，而本機自動瀏覽器工具先前多次受 Windows sandbox、viewport、profile 與截圖可信度限制影響。由使用者手動檢驗可避免工具雜訊拖慢設計迭代，也更符合此網站以本人審美為準的性質。
- **影響範圍**：
  - `.agents/skills/professional/development_standards.md`
  - `.agents/architecture/technical_architecture.md`
  - 所有後續 UI、responsive layout、視覺品質與設計迭代任務
- **後續 Agent 行動**：
  - UI 變更後先完成 build 與可機械檢查，並告知使用者可手動預覽。
  - 不因無法完成自動截圖而卡住任務，也不把 headless 截圖當成最終視覺判斷。
  - 收到使用者手動檢驗回饋後，優先依回饋修正實際頁面。

### D-2026-06-11-006 - 自動化視覺驗證需先確認工具可信度

- **日期**：2026-06-11
- **狀態**：已由 `D-2026-06-11-007` 修訂
